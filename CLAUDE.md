...
- **HTML in markdown**: When writing HTML tags in `.md` files, always wrap them in backticks (e.g. `<div>`, `<select>`) to prevent rendering issues in markdown preview panes.
- **Inline code spans**: Always delimit inline code with double backticks (` ``code`` `), never single backticks. This handles content that itself contains a literal backtick (e.g. a JS template literal) without needing a backslash escape — Markdown code spans don't process backslash escapes, so escaping a backtick inside a single-backtick span silently closes the span early and corrupts the rendered output.
...
- **Trim persuasive exposition**: State documentation rules directly — don't narrate how a decision was reached, defend it against a hypothetical skeptic, or explain what was previously assumed. Add reasoning only when it resolves genuine ambiguity in a case the rule doesn't cover. Example: prefer "Bridge every React Aria `data-*` attribute to its corresponding Bootstrap styles" over a paragraph justifying *why* (past failures, cost/benefit tradeoffs, case-by-case risk) — that reasoning belongs in a commit message or tracking doc, not the rule itself.
- **Artifact templates**: Before writing a template or structural spec for any project artifact, read at least one real example of that artifact if one exists. If no example exists yet, explicitly flag the template as speculative — it has not been ground-truthed against real output and may need revision once the first real instance is created.
- **Tool failure**: When any tool call fails — including load failures for deferred tools — stop and report the error before continuing. Do not proceed as if the step completed or silently work around the missing capability. In sub-agent contexts, tool unavailability is a blocking condition to surface immediately, not an obstacle to route around.
...
- **Session transcripts**: When the user says "session transcript", "chat transcript", "transcribe", or similar, produce a verbatim transcript of the current session's spoken dialogue — user turns and agent turns only. Do not paraphrase, abbreviate, or use bracketed summaries for any turn regardless of length — reproduce the exact text of each turn as delivered. Omit tool outputs, terminal views, and collapsed thinking blocks, except where the dialogue directly quotes or references specific content from those sources. Enclose the transcript in a code block using **ten backticks** — not the four from the Markdown summaries rule above. A transcript can quote dialogue that itself contains fenced code blocks (e.g. a triple-backtick SCSS/TSX example quoted mid-turn); four backticks was tested against real transcript content on 2026-07-21 and still closed early despite exceeding the deepest nested fence (3 backticks), so ten is a deliberately wide margin rather than a tight "one more than the deepest nesting" calculation.
