...
- **Taxonomy:** `.accordion > .accordion-item` is **direct descendant** (Section 1 row 1). `.accordion-header .accordion-button` is written as a **nested descendant** (Section 1 row 2), even though the two happen to be immediate parent/child in Bootstrap's own markup. `.accordion-button:not(.collapsed)` is **pseudo-class stacked with class** (Section 1 row 5). **What breaks:** if `.accordion-button` is placed as a sibling of `.accordion-header` instead of nested inside it, `.accordion-header .accordion-button` no longer matches — the button keeps working (JS toggling is wired via `data-bs-toggle`/`data-bs-target`, not this selector) but loses its caret rotation and expanded/collapsed background styling, since those all key off this same descendant chain (see states.md).
